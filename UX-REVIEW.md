...
| 7 | **Catalog picker → one searchable picker** | Three native selects over 130 calibers; no type-ahead, poor on mobile, can't compare loads while choosing | M–L | ✅ Done — the smallest-viable-version filterable-combobox cascade shipped first (§5), then the full flattened-search rebuild landed 2026-09-13 (PR #29) once the catalog grew large enough to justify it (855→~1,852 loads, 8→12 manufacturers). One search box over the whole catalog, caliber/manufacturer demoted to optional additive filter chips, hand-rolled relevance scoring, desktop popover / mobile full-screen sheet sharing one row renderer, `onSelect(ammo)` contract unchanged across all 5 call sites. See HANDOFF.md's 2026-09-13/14 Shipped entry for the full writeup. |
...
- **Smallest viable version** if a rebuild isn't on the table: swap each native `<select>`
  ...
  **✅ This is the version Jake picked and shipped first** — substring-match combobox with
  full keyboard nav and ARIA wiring, cascade/auto-advance logic untouched. Per-step result
  count wasn't added. **The full flattened-search rebuild (one box over the whole catalog)
  shipped later, 2026-09-13 (PR #29)** — see the item #7 summary row above.
